...
var manager={patientId:null,tab:'board',search:'',category:'Todas',previewCategory:'Todas',previewPhrase:[],customImage:null,customAudio:null,recording:false,recorder:null,chunks:[]};
...
function defaultSettings(){return {volume:.9,rate:.95,voiceName:'',speakOnTap:true,addToPhrase:true,columns:4,cardSize:'normal'};}
...
function openManager(pid){manager.patientId=pid;manager.tab='board';manager.previewCategory='Todas';manager.previewPhrase=[];renderManager();}
...
function renderUsage(p){
  var u=p.usage||[],today=new Date().toISOString().slice(0,10),tu=u.filter(function(e){return String(e.at||'').slice(0,10)===today}),taps=tu.filter(function(e){return e.type==='card';}),phr=tu.filter(function(e){return e.type==='phrase';}),counts={};taps.forEach(function(e){counts[e.label]=(counts[e.label]||0)+1;});var top=Object.keys(counts).sort(function(a,b){return counts[b]-counts[a];}).slice(0,8);
  return '<section class="caa-panel"><h3>Uso de hoje</h3><p>Dados descritivos da comunicação. A interpretação clínica continua sendo do profissional.</p><div class="caa-stats"><div class="caa-stat"><small>INTERAÇÕES</small><b>'+tu.length+'</b></div><div class="caa-stat"><small>CARTÕES</small><b>'+taps.length+'</b></div><div class="caa-stat"><small>FRASES</small><b>'+phr.length+'</b></div><div class="caa-stat"><small>PALAVRAS DIFERENTES</small><b>'+Object.keys(counts).length+'</b></div></div><div class="caa-grid-2"><div><h3>Mais utilizados</h3><div class="caa-history">'+(top.length?top.map(function(k){return '<div class="caa-history-item"><b>'+esc(k)+'</b><span>'+counts[k]+' uso'+(counts[k]===1?'':'s')+'</span></div>';}).join(''):'<div class="caa-empty"><b>Sem uso registrado hoje</b></div>')+'</div></div><div><h3>Frases recentes</h3><div class="caa-history">'+(phr.length?phr.slice(-10).reverse().map(function(e){return '<div class="caa-history-item"><b>'+esc(e.text)+'</b><span>'+dateTime(e.at)+'</span></div>';}).join(''):'<div class="caa-empty"><b>Nenhuma frase registrada</b></div>')+'</div></div></div></section>';
}
...
function communicatorHTML(p,snapshot,preview){
  ...
  return '<div class="caa-sentence"><div class="caa-sentence-items" data-phrase-items>'+((preview?manager.previewPhrase:[]).map(function(c){return '<span class="caa-word-chip">'+esc(c.label)+'</span>';}).join('')||'<span style="color:#a99a94">Monte uma frase…</span>')+'</div><button class="caa-btn" data-preview-back>⌫</button><button class="caa-btn" data-preview-clear>Limpar</button><button class="caa-btn primary" data-preview-speak>🔊 Falar</button></div><div class="caa-categories">'+cats.map(function(c){return '<button class="caa-category '+(c===cat?'active':'')+'" data-preview-cat="'+esc(c)+'">'+esc(c)+'</button>';}).join('')+'</div><div class="caa-board" style="--caa-cols:'+cols+'">'+shown.map(function(c){return '<button class="caa-comm-card" data-preview-card="'+esc(c.id)+'" style="background:'+esc(c.color||'#fff')+'"><img src="'+esc(c.image)+'" alt=""><b>'+esc(c.label)+'</b></button>';}).join('')+'</div>';
}
...
function bindManager(w,p){
  w.onclick=function(e){
    ...
    var pc=e.target.closest('[data-preview-card]');if(pc){var c=cardById(profile(manager.patientId),pc.getAttribute('data-preview-card'));if(c){var pr=profile(manager.patientId);if(pr.settings.addToPhrase)manager.previewPhrase.push(c);if(pr.settings.speakOnTap&&window.FonelyCAASpeech)window.FonelyCAASpeech.speakCard(c,pr.settings);renderManager();}return;}
    ...
    if(e.target.closest('[data-preview-clear]')){manager.previewPhrase=[];renderManager();return;}
    if(e.target.closest('[data-preview-back]')){manager.previewPhrase.pop();renderManager();return;}
    if(e.target.closest('[data-preview-speak]')){var txt=manager.previewPhrase.map(function(c){return c.label;}).join(' ');if(txt&&window.FonelyCAASpeech)window.FonelyCAASpeech.speak(txt,profile(manager.patientId).settings);return;}
  ...
  setting('#caaVolume','volume',function(el){return Number(el.value)/100;});setting('#caaRate','rate',function(el){return Number(el.value)/100;});setting('#caaVoice','voiceName');setting('#caaSpeakTap','speakOnTap',function(el){return el.checked;});setting('#caaAddPhrase','addToPhrase',function(el){return el.checked;});setting('#caaColumns','columns',function(el){return Number(el.value);});

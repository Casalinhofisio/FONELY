...
Deno.serve(async (req: Request) => {
  ...
  const memberResult = await admin.from("clinic_members")
    .select("id,owner_id,status").eq("user_id", caller.id).eq("status", "active")
  ...
  if (action === "invite") {
    ...
    if (existingMembership.data?.status === "active") return json({ error: "Esse profissional já faz parte da equipe." }, 409);
    ...
    if (!targetUser) {
      const invite = await admin.auth.admin.inviteUserByEmail(email, {
        redirectTo: "https://casalinhofisio.github.io/FONELY/",
    ...
    const row = {
      owner_id: ownerId, user_id: targetUser.id, email, name, role, permissions, status: "active",
      invited_by: caller.id, invited_at: new Date().toISOString(), updated_at: new Date().toISOString()
    };
    ...
    return json({ ok: true, member: saved.data, invitation_sent: invitationSent, existing_login: !invitationSent });
  }
  ...
  if (action === "update") {
    ...
    const updated = await admin.from("clinic_members").update(patch)
      .eq("id", memberId).eq("owner_id", ownerId).eq("status", "active").select().maybeSingle();
    ...
    return json({ ok: true, member: updated.data });
  }

  if (action === "remove") {
    const memberId = String(body?.member_id || "");
    if (!memberId) return json({ error: "Profissional não informado." }, 400);
    const removed = await admin.from("clinic_members").update({ status: "removed", updated_at: new Date().toISOString() })

...
function boardStamp(board){
  ...
  return String(board.updated_at||snap.publishedAt||snap.version||'');
}
...
async function fetchBoard(){
  ...
  try{
    ...
    var oldStamp=state.lastStamp||boardStamp(state.board),newStamp=boardStamp(d.board);
    var changed=!state.board||oldStamp!==newStamp;
    cacheBoard(d.board);
    cacheAACImages(d.board);
    state.board=d.board;
    state.lastStamp=newStamp;
    state.offline=false;
    return changed;
...
async function sync(force){
  ...
  try{
    ...
    var hadBoard=!!state.board,c=cached();
    if(c){
      var wasOffline=state.offline;
      state.board=c;state.lastStamp=boardStamp(c);state.offline=true;
      if(force||!hadBoard||!wasOffline)render();
...
function startAutoSync(){
  ...
  state.pollTimer=setInterval(function(){
    if(!document.hidden&&navigator.onLine)sync(false);
  },5000);
}

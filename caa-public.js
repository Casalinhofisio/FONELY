...
function render(){
  ...
  var snap=p.published,cards=snap.cards||[],cats=['Todas'].concat(Array.from(new Set(cards.map(function(c){return c.category;})))),shown=state.category==='Todas'?cards:cards.filter(function(c){return c.category===state.category;}),vol=Math.round(((state.volumeOverride==null?(snap.settings&&snap.settings.volume):state.volumeOverride)||0)*100);
